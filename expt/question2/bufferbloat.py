...
def start_iperf(net):
    ...
    h3 = net.get('h3')
    h4 = net.get('h4')
    h3.popen("iperf -s -u -p 5566 -i 1 > %s/server.out"%(args.dir),shell =True)
    h4.popen("iperf -c %s 10M -u -b 10M -t 25 -p 5566  > %s/iperf1.out" % (h3.IP(), args.dir), shell=True)
...
def start_ping(net):
    ...
    h4 = net.get('h4')
    #h3 = net.get('h3')
    popen = h4.popen("ping -i 0.1 %s > %s/ping1.txt"%(h3.IP(), args.dir), shell=True)
